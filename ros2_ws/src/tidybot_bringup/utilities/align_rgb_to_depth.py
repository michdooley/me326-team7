#!/usr/bin/env python3
"""
RGB-to-Depth alignment utility for RealSense cameras.

Aligns an RGB image to a depth image using camera intrinsics and an
extrinsic transform between the two sensors. This preserves the original
depth data (critical for 3D tasks like GraspNet) by only warping the
lower-fidelity color image.

Usage as a library (from another script):
    import sys
    from pathlib import Path
    sys.path.insert(0, str(Path(__file__).resolve().parent))
    from align_rgb_to_depth import align_rgb

    aligned_rgb = align_rgb(rgb, depth)

Usage from the command line:
    python3 align_rgb_to_depth.py rgb.png depth.png aligned_rgb.png
...
def convert_intrinsics(img: np.ndarray,
                       K_old: np.ndarray,
                       K_new: np.ndarray,
                       new_size: Tuple[int, int] = (640, 480)) -> np.ndarray:
    """
...
def align_rgb(rgb: np.ndarray,
    ...
    """
    Align an RGB image to a depth image.

    Warps the RGB image from RGB camera intrinsics into the depth camera's
    frame, preserving the depth data untouched.

    Parameters:
        rgb: RGB image (H, W, 3), uint8.
        depth: Depth image, used only for its dimensions.
        ...
        cam2cam_transform: 4x4 extrinsic transform from RGB to depth camera.
                           Defaults to identity.

    Returns:
        RGB image aligned to the depth camera frame.
    """
    if cam2cam_transform is None:
        cam2cam_transform = np.eye(4)

    old_fx, old_fy, old_cx, old_cy = rgb_K
    new_fx, new_fy, new_cx, new_cy = depth_K
    ...
    # Re-project RGB from RGB intrinsics to depth intrinsics
    rgb_aligned = convert_intrinsics(rgb, K_old, K_new,
                                     new_size=(depth.shape[1], depth.shape[0]))

    # Warp using the extrinsic transform between cameras
    rgb_aligned = warp_image(rgb_aligned, K_new,
...
def main():
    parser = argparse.ArgumentParser(
        description="Align an RGB image to a depth image (preserves depth data)."
    )
    parser.add_argument("rgb", help="Path to RGB image")
    parser.add_argument("depth", help="Path to depth image (for dimensions)")
    parser.add_argument("output", help="Path for aligned RGB output")
