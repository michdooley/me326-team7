...
Usage:
    ros2 run tidybot_bringup task_navigate_grasp_return_no_obstacles.py
    ros2 run tidybot_bringup task_navigate_grasp_return_no_obstacles.py --ros-args -p sim:=false
    ros2 run tidybot_bringup task_navigate_grasp_return_no_obstacles.py --ros-args \
...
class NavigateGraspReturnNoObstaclesNode(NavigationTaskNode):
    """Navigate to target, grasp it via top-down pick, then return home.

    All obstacle detection and avoidance is disabled.
    """
    ...
    # Stop distance for approach
    SEEK_TARGET_STOP_DISTANCE_M = 0.20
    APPROACH_STOP_DISTANCE_BUFFER_M = 0.05

    # Persistent target tracking
    APPROACH_LOST_FRAMES = 40
    APPROACH_TARGET_MEMORY_S = 4.0
    YOLO_DETECTION_MAX_AGE_S = 3.0
    ...
    def __init__(self):
        ...
        self.get_logger().info(
            f'NavigateGraspReturnNoObstacles: arm={self.grasp_arm}, '
            f'target={self.grasp_target_name}, sim={self.is_sim}, '
            f'voice_cmd={self.use_voice_command}, obstacles=DISABLED')

    # ------------------------------------------------------------------
    # Disable ALL obstacle detection and avoidance
    # ------------------------------------------------------------------

    def is_obstacle_blocking(self) -> bool:
        """Always returns False — no obstacle avoidance."""
        return False

    def _handle_approach_obstacle(self):
        """No-op — obstacles are ignored."""
        pass

    def _handle_simple_yolo_approach(self):
        """Dead-simple approach: see banana -> turn toward it -> drive straight.
        No obstacle checks whatsoever."""
    ...
    GRASP_CAMERA_TILT_RAD = 0.65  # ~37 deg down
    ...
    def _execute_grasp(self):
        ...
        # Reset camera tilt for grasping
    ...
    def _get_depth_image(self):
        """Get current depth image as float32 array in mm."""
    ...
    def _wait_for_future(self, future, timeout_sec=10.0):
        """Wait for service future to complete."""
    ...
    # ------------------------------------------------------------------
    # Return-home logic (no obstacle checks)
    # ------------------------------------------------------------------
    ...
    def _handle_return_home(self):
        ...
        # Drive straight home — no obstacle checks
        self.publish_pose_target(self.home_x, self.home_y, self.home_theta)


def main(args=None):
    rclpy.init(args=args)
    node = NavigateGraspReturnNoObstaclesNode()
