...
def method_minwidth_sweep(
    ...
    **kwargs,
    ...
    """Max-clearance sweep: picks the angle + center that maximizes the
    minimum clearance between object and gripper fingers.

    For each angle, computes the optimal grasp center (midpoint of object
    extent along finger axis) so the object is centered in the gripper.
    Score = min_clearance / (max_opening / 2), normalized [0,1].
    """
    ...
    for angle_deg in range(0, 180, angle_step):
        ...
        # Optimal center offset along finger axis (centers object in gripper)
        ...
        # Adjust grasp center from centroid along finger axis
...
def load_snapshot(path: Path) -> dict:
    ...
    for key in ['z_offset', 'z_bias', 'pre_grasp_height', 'target_object', 'floor_z',
                 'grasp_pose', 'pre_grasp_pose', 'grasp_yaw', 'grip_width']:
        if key in data:
            val = data[key]
...
class GraspVisualizer:
    def __init__(self, points_base: np.ndarray, params: dict = None):
        ...
        # Store executed grasp pose if available (7-element: x,y,z,qw,qx,qy,qz)
        self.executed_grasp = self.params.pop('grasp_pose', None)
        self.executed_pregrasp = self.params.pop('pre_grasp_pose', None)
        self.executed_yaw = self.params.pop('grasp_yaw', None)
        self.executed_width = self.params.pop('grip_width', None)
    ...
    def _show_executed_grasp(self, visible: bool):
        """Show or hide the executed grasp pose from the snapshot."""
        # Remove old handles
        for h in getattr(self, '_executed_handles', []):
            h.remove()
        self._executed_handles = []

        if not visible or self.executed_grasp is None:
            return

        yaw = float(self.executed_yaw) if self.executed_yaw is not None else 0.0
        width = float(self.executed_width) if self.executed_width is not None else 0.04

        # Grasp pose: x,y,z,qw,qx,qy,qz
        g = self.executed_grasp
        mesh = create_wx250s_gripper_mesh(
            g[:3], yaw, width, color=(0, 220, 220), alpha=220)
        h = self.server.scene.add_mesh_trimesh(
            name='/executed/grasp', mesh=mesh)
        self._executed_handles.append(h)

        # Pre-grasp pose
        if self.executed_pregrasp is not None:
            pg = self.executed_pregrasp
            pg_mesh = create_wx250s_gripper_mesh(
                pg[:3], yaw, width, color=(0, 220, 220), alpha=80)
            h2 = self.server.scene.add_mesh_trimesh(
                name='/executed/pregrasp', mesh=pg_mesh)
            self._executed_handles.append(h2)
    ...
    def run(self):
        ...
        with self.server.gui.add_folder('Executed Grasp'):
            show_exec_cb = self.server.gui.add_checkbox(
                'Show executed grasp', initial_value=self.executed_grasp is not None)
            show_exec_cb.on_update(lambda _: self._show_executed_grasp(show_exec_cb.value))
            if self.executed_grasp is None:
                show_exec_cb.disabled = True
        ...
        if snapshots:
            def do_load_snapshot(_=None):
                ...
                skip_keys = {'points_base', 'points_base_raw',
                             'points_full_scene', 'grasp_pose',
                             'pre_grasp_pose', 'grasp_yaw', 'grip_width'}
                ...
                self.executed_grasp = snap.get('grasp_pose')
                self.executed_pregrasp = snap.get('pre_grasp_pose')
                self.executed_yaw = snap.get('grasp_yaw')
                self.executed_width = snap.get('grip_width')
                self.points_base, new_params = self._ensure_floor_filtered(
                    snap['points_base'], new_params)
                self.params.update(new_params)
                # Update auxiliary clouds/markers if visible
                self._show_raw_cloud(show_raw_cb.value)
                self._show_full_scene(show_full_cb.value)
                self._show_executed_grasp(show_exec_cb.value)
                show_full_cb.disabled = self.points_full_scene is None
                show_exec_cb.disabled = self.executed_grasp is None
        ...
        # Initial computation
        do_recompute()
        if self.executed_grasp is not None:
            self._show_executed_grasp(True)
