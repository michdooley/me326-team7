#!/usr/bin/env python3
"""
TidyBot2 Bimanual Shelf Demo - Pre-computed Joint Space Trajectories

Demonstrates both WX250s arms simultaneously picking up objects from the ground,
driving toward a shelf, and placing them on top. Uses Mink IK for offline waypoint
pre-computation, then smooth joint-space interpolation during the simulation.

Usage:
    cd simulation/scripts
    uv run python shelf_demo_sim.py
"""
...
GRIPPER_OPEN = 0.037
RIGHT_GRIPPER_CLOSED = 0.025  # cylinder radius 0.019 + finger thickness
LEFT_GRIPPER_CLOSED = 0.021   # box half-width 0.015 + finger thickness
...
def main():
    ...
    model_path = script_dir / "../assets/mujoco/scene_shelf_demo.xml"
    ...
    print(f"Right object position: {right_obj_pos}")
    print(f"Left object position:  {left_obj_pos}")
    ...
    right_approach = np.array([right_obj_pos[0], right_obj_pos[1], right_obj_pos[2] + 0.15])
    left_approach = np.array([left_obj_pos[0], left_obj_pos[1], left_obj_pos[2] + 0.15])
    ...
    right_lift = np.array([0.32, right_obj_pos[1], 0.40])
    left_lift = np.array([0.32, left_obj_pos[1], 0.40])

    shelf_place_x = 0.85
    shelf_z = 0.30
    right_place = np.array([shelf_place_x, -0.10, shelf_z + 0.08])
    left_place = np.array([shelf_place_x, 0.10, shelf_z + 0.08])
    ...
    # ==========================================================================
    print("Starting bimanual shelf demo...")
